...
const CounterSalesReportPDF: FC = () => {
  ...
  const generatePDF = async () => {
    ...
    pdf.addImage(imgData, "PNG", 0, 0, imgWidth, finalHeight);
    pdf.save("Counter_Sale_Report.pdf");
  };
                  ...
                  <tbody>
                    {reportData.map((row, index) => (
                      <tr key={index}>
                        ...
                        <td style={{ border: '1px solid grey', textAlign: 'center', padding: '8px' }}>{row.totalQuantity}</td>
                        <td style={{ border: '1px solid grey', textAlign: 'center', padding: '8px' }}>{row.taxable}</td>
