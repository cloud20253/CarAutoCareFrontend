...
export default function InvoiceForm() {
  ...
  const handlePartsChange = (
    ...
    // Prevent editing for non‑editable tax fields
    if (['cgstPercent', 'sgstPercent', 'igstPercent'].includes(field)) return;
  ...
  const fetchVehicleData = async () => {
    ...
    try {
      ...
      // Map response fields including cgst and sgst
      const parts = partsArray.map((p: any) => ({
        ...
        cgstPercent: p.cgst !== undefined ? String(p.cgst) : "",
        sgstPercent: p.sgst !== undefined ? String(p.sgst) : "",
        igstPercent: "" // assume IGST not provided
      }));
  ...
  const handleSubmit = async () => {
    // Open a blank window immediately upon click
    ...
    try {
      ...
      // Write an HTML document with an embed tag to display the PDF
      if (newWindow) {
  ...
  // A common style for disabled tax fields (grey background)
              ...
              {formData.parts.map((item, index) => {
                ...
                return (
                  <TableRow key={index}>
                    ...
                    {/* Render non-editable tax fields with grey background */}
                    <TableCell align="center">
                      <TextField
                        name="cgstPercent"
                        type="number"
